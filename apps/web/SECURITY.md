...
### Door list / GET /api/doors (when added)

- **Pagination** — never return unbounded lists. Cursor-based is safer than offset for spam scenarios.
- **Don't expose internal IDs** in URLs if you can help it; use shortcodes or slugs.
- **Rate limit reads too**, just less aggressively than writes (e.g. 60 reads/min/IP).
- **Privacy:** a door's lat/lng can be a fingerprint of where someone went. Consider: do we expose exact coords publicly, or snap to nearest block?
